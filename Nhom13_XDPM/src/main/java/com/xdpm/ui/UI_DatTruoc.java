...
public class UI_DatTruoc extends JPanel {
	...
	public UI_DatTruoc() {
		...
		pnlKhachHang
				.setBorder(new TitledBorder(null, "Khách hàng", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		btnFindCustomer.addActionListener(e -> {
			try {
				if (validCustomer()) {
					...
					if (customer == null) {
						JOptionPane.showMessageDialog(null, "Không tìm thấy khách hàng này!!", "Lỗi",
		...
		btnDatTruoc.addActionListener(e -> {
			if (validData()) {
				...
				if (diskDAO.checkDiskOnShelfByIdTitle(titleID).size() > 0) {
					JOptionPane.showMessageDialog(null, "Đĩa vẫn còn trên kệ!!", "Lỗi", JOptionPane.ERROR_MESSAGE);
					return;
				}
				...
				if (!reservationDAO.checkReservationRecord(cusID, titleID)) {
					...
					JOptionPane.showMessageDialog(null, "Khách hàng đã đặt tựa đĩa này!!", "Lỗi",
							JOptionPane.ERROR_MESSAGE);
				}
		...
		btnHuy.addActionListener(e -> {
			if (JOptionPane.showConfirmDialog(this, "Hủy đặt trước?", "Chú ý",

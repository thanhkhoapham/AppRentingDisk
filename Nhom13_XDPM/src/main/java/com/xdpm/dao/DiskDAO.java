...
public class DiskDAO extends AbstractCRUD<Disk> {
	...
	public List<Disk> getListOfRentableDisk(int titleID) {
		...
		Query query = entityManager.createQuery(
				"from Disk where titleID = :titleID and status = 'onShelf' and enabled = true", Disk.class);
		query.setParameter("titleID", titleID);
	...
	public List<Disk> checkDiskOnShelfByIdTitle(int id) {

		Query query = entityManager.createQuery("from Disk where status='onShelf' and titleID=:id and enabled=1",
				Disk.class);
